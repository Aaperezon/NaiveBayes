...
datasets_filenames = [
    ("imdb_labelled.txt","imdb_confusion_matrix.csv","imdb_labelled_test.csv")
    ,("amazon_cells_labelled.txt","amazon_cells_confusion_matrix.csv","amazon_cells_labelled_test.csv")
    ,("yelp_labelled.txt","yelp_confusion_matrix.csv","yelp_labelled_test.csv")
]

for dataset_filenames in datasets_filenames:
    ...
    def getTrainingAndEvaluationTexts(texts, trainingRate):
        texts = [(txt,i) for i,txt in enumerate(texts)]
        shuffle(texts)
    ...
    def getTableOcurrencesOfWords(texts):
        ...
        for i in range(len(texts)):
            class_f[texts[i][0][1]] += 1
            words = texts[i][0][0].split()
            for word in words:
                if word in counts:
                    counts[word] += 1
                else:
    ...
    def getOcurrenceByClass(attempted_string,attempted_class,texts):
        counter = 0
        for i in range(len(texts)):
            if(int(texts[i][0][1]) == attempted_class): 
                counter = counter + texts[i][0][0].split().count(attempted_string)
        return counter
    ...
    file1 = open(dataset_filenames[0], 'r')
    ...
    def getTestTable(tests, frequency_class,negatives, positives,  v):
        ...
        for test in tests:
            ...
            for word in test[0][0].split():
                if(word in log_[test[0][1]] ):
                    log_[test[0][1] ][word] += 1
                else:
                    log_[test[0][1] ][word] = 1
            ...
            calc_class,real_class = int(log_v[1]<log_v[0]),test[0][1]
            test_table.append([test[1],log_v[0], log_v[1],calc_class,real_class])
            confusion_matrix[real_class][calc_class]+=1
        return test_table,confusion_matrix
    ...
    test_table,confusion_matrix = getTestTable(training_texts, words_ocurrences_dict,negative_class_p, positive_class_p,vocabulary_len)

    for i in test_table:
        print(i)
    print(confusion_matrix)

    pd.DataFrame(data=confusion_matrix).to_csv(dataset_filenames[1])

    pd.DataFrame(data=test_table).to_csv(dataset_filenames[2],header=False,index=False)
    #print(df)
